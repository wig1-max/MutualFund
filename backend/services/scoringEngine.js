  ...
  const insertRecs = db.transaction((recs) => {
    ...
    recs.forEach((rec) => {
      stmt.run({
        ...
        category_fit_score: rec.category_fit_score ?? 0,
        risk_alignment_score: rec.risk_alignment_score ?? 0,
        tax_efficiency_score: rec.tax_efficiency_score ?? 0,
        overlap_penalty: rec.overlap_penalty ?? 0,
        quality_score: rec.quality_score ?? 0,
